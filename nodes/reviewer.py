...
def review_prompt_progress(state: PromptState) -> str:
    ...
    return (
        ...
        f"- 처음부터 있던 요소: {', '.join(initial_elements) if initial_elements else '없음'}\n"
        f"- 현재 채워진 요소: {len(present_elements)}/8 ({', '.join(present_elements) if present_elements else '없음'})\n"
