...
- 애매하게 추론해야 하는 요소는 value를 채우지 말고 null로 둔다.
- 하나의 정보는 가장 적절한 요소 하나에만 배치하고, 여러 요소에 반복해서 넣지 않는다.
- 여러 정보가 한 요소에 들어가면 세미콜론(;)으로 구분해 하나의 문자열로 요약한다.
- value는 가능하면 짧게 작성하고, 여러 정보가 있을 때만 세미콜론으로 구분한다.
- 예: "자료구조 기초 이수; 알고리즘 대회 경험 없음; 한국 취업 준비생"

좋은 예 / 나쁜 예:
- 나쁜 domain: "학습"
  좋은 domain: "코딩 테스트 교육"
  이유: 너무 넓은 분야보다 사용자의 목적이 드러나는 분야가 좋다.
- 나쁜 value: 사용자 프롬프트 전체를 그대로 복사
  좋은 value: "문제를 worked example로 설명"
  이유: value는 원문 복사본이 아니라 요소별 핵심 의미 요약이다.
- 나쁜 추측: "취업 준비 도와줘"를 "3개월 안에 대기업 백엔드 신입 취업 준비"로 채우기
  좋은 처리: goal.value는 "취업 준비 도움", context.value는 null
  이유: 기간, 회사군, 직무는 사용자가 말하지 않았으므로 추측하면 안 된다.
